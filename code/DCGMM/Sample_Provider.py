...
class SampleProvider(object):
  ...
  def _transform(self, images_org):
    ...
    if self.image_options["crop"]:
        ...
        print(resize_size//2)
        print(images_org.shape[0]-resize_size//2)
        y = np.random.permutation(range(resize_size//2, images_org.shape[0]-resize_size//2 + 1))
        y = int(y[0])
        y1 = int(y - resize_size/2.0)
        y2 = int(y + resize_size/2.0)
        
        x = np.random.permutation(range(resize_size//2, images_org.shape[1]-resize_size//2 + 1))
        x = int(x[0])
